...
<%
	...
	Connection con = db.getConnection();
		switch(request.getParameter("custSubmitType")){
	    	case "custBuyReserve":
	    		if(request.getParameter("custBuyReserveFlight1").equals("No Flights Available")){
	    			...
	    			PreparedStatement psAddTicket = 
		               ...
		               /*  PreparedStatement psUsername = con.prepareStatement("Select Username from users where UserID = ?");
						...
						rsUsername.next(); */
						String username = request.getParameter("custFlightsBuyUsername");
		    		...
		    		int numFlights = Integer.parseInt(request.getParameter("custNumFlightsBuy"));
		    		int totalCost = 10;
		    		ArrayList<ArrayList<Integer>> availableSeats = new ArrayList<ArrayList<Integer>>();
					for(int i = 0; i < numFlights; i++){
						availableSeats.add(new ArrayList<Integer>());
						String flightData = request.getParameter("custBuyReserveFlight"+(i+1));
			    		String flightClass = request.getParameter("custBuyClass"+(i+1));
...
/* 								out.println(Integer.parseInt(rsClassCosts.getString("firstClassPrice")));
					...
					ps.executeUpdate();
		    		out.println("<br>Your total cost is $" + totalCost);
	    		}
	    		break;
	    	case "custEditReserve": 
	    		String ticketNum = request.getParameter("custEditTicketNum");
	    		if(request.getParameter("custDeleteTicket").equals("yes")){
	    			try{
	    			...
	    			if(request.getParameter("custEditReserveClass").equals("No Flights Available")){
		    			out.println("Sorry, no flights available, please return home");
		    		}else{
		    			String flightClass = request.getParameter("custEditClass");
		    			String classFlightData = request.getParameter("custEditReserveClass");
		    			...
		    			//Remove Flight
		    			String removeFlightData = request.getParameter("custRemoveReserveClass");
		    			if(!removeFlightData.equals("None")){
							...
							String username = request.getParameter("custEditUsername");
